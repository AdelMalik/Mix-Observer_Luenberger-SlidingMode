"""
Case 3 — Wilson–Cowan neural mass, sigmoid nonlinearity.
Sector-bounded LMI (Gamma = lam_sig/4 * I), sliding-mode bank.

Observer:  dV̂/dt = A V̂ + W S(V̂) + K(y - C V̂) - K'(C W S(V̂) - ẑ₂)
           with ẑ₂ reconstructed from y via sliding-mode bank.

WC form: A = -lambda*I, CWS(V) reconstructed.
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# LMI: sector-bounded, WC form — P(I-K'C)W in (1,2)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
print("=" * 70)
print("Case 3 — Lipschitz sigmoid + sliding-mode bank")
print("Sector-bounded LMI  (Gamma = lam_sig/4 = 1.0)")
print("=" * 70)
...
def rhs_classical(vh, K, ym):
    return A @ vh + W_sw @ S(vh, LAM_SIG) + K @ (ym - C @ vh)

def rhs_combined(vh, K, Kp, ym, z2_est):
    return (A @ vh + W_sw @ S(vh, LAM_SIG)
            + K @ (ym - C @ vh)
            - Kp @ (C @ (W_sw @ S(vh, LAM_SIG)) - z2_est))
...
for sig in sigmas:
    ...
    nr    = Nt_sw // stride
    e_s   = np.zeros(nr); e_c = np.zeros(nr); e_smo = np.zeros(nr)
    rec   = 0
...
fig.tight_layout()
fig.savefig(f"{OUT}\\sigmoid_noise_sweep.pdf")
plt.close(fig)

print(f"\nFigure saved to {OUT}\\sigmoid_noise_sweep.pdf")

# ── Time-domain error trajectory ─────────────────────────────────────────
T_END = 8.0; Nt_td = int(T_END / dt); sig_td = 0.3
...
for k in range(Nt_td):
    ...
    V = rk4(lambda v, n_: A @ v + W_sw @ S(v, LAM_SIG) + n_, V, dt,
...
fig2.tight_layout()
fig2.savefig(f"{OUT}\\sigmoid_error_trajectory.pdf")
plt.close(fig2)
print(f"Figure saved to {OUT}\\sigmoid_error_trajectory.pdf")
